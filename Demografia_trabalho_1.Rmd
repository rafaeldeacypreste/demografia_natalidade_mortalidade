...
As probabilidades de um recém-nascido sobreviver aos 5 anos em Pernambuco entre os anos de 2000 e 2016 estão na tabela \@ref(tab:ProbCincoAnos) abaixo. 
...
As probabilidades de um recém-nascido sobreviver ao primeiro aniversário em Pernambuco entre os anos de 2000 a 2020 estão na tabela \@ref(tab:ProbUmAno) abaixo. 
...
Analisando a Figura \@ref(fig:lexisDiagram), percebe-se que em Pernambuco houve decréscimo no número de óbitos entre os anos de 2000 a 2021, enquanto houve crescimento no número de nascidos vivos no mesmo período.
...
Já a **Taxa de Fecundidade Geral (TFG)** refere-se à divisão, em um determinado ano, do total de nascidos vivos pela população feminina em período fértil (15 a 49 anos de idade). Em Pernambuco, essa taxa foi de `r media_nascimentos_19_21/ pop_fem_fertil`.
...
A **Taxa de Fecundidade Total (TFT)** é obtida pelo somatório das taxas específicas de fecundidade para as mulheres em idade fértil residentes de uma região específica. A TFT para o estado de Pernambuco corresponde a `r (nascimentos_total_21 / 382499) + (nascimentos_total_21 / 407176) + (nascimentos_total_21 / 393940) + (nascimentos_total_21 / 403316) + (nascimentos_total_21 / 368470) + (nascimentos_total_21 / 328946)`.
...
A **Taxa Bruta de Reprodução** assemelha-se à Taxa Bruta de Natalidade, diferenciando-se desta última apenas por considerar somente o número de **filhas** nascidas vivas. O valor desta taxa, calculada para o estado de Pernambuco por mil habitantes é `r (media_nasc_fem_19_21 / pop_fem_fertil) * 1000`.

A **Taxa Líquida de Reprodução** relaciona-se ao número de filhas tidas, por mulher, durante sua idade reprodutiva. Para o estado de Pernambuco, este valor é `r mean(nascimentos_fem_21 / pop_fem_fertil)`.
...
graf_dist_idade_esc <-
  ...
  geom_vline(
    data = vline_df,
    aes(xintercept = medianas),
    linetype = "dashed",
    colour = "red4"
  ) +
  facet_grid(rows = vars(factor(escolaridade))) +
