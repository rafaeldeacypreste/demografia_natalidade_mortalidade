...
```{r, echo=FALSE}
# Como a conexão com a internet não é a melhor opção sempre para gerar o mkd
# melhor fazer uma gambiarra por trás para ler mais rapidamente
# Com isso, após baixar a primeira vez, a leitura de um objeto será direta

# Primeira rodada
# 
# dados_sinasc %>% 
#   fwrite(file = caminho_tabela)

library(data.table)


caminho_tabela <- 
 file.path(getwd(), "Demografia", "Atividades", "dados_sinasc.csv")

dados_sinasc <- fread(caminho_tabela, encoding = "UTF-8")
...
```{r, echo=FALSE}
# PAsso para não preicsar baixar o tempo todo:
# 
caminho_tabela <- 
  file.path(getwd(), "Demografia", "Atividades", "dados_sinasc_2021.csv")
# 
# dados_sinasc_2021 %>% 
#     fwrite(file = caminho_tabela)

dados_sinasc_2021 <- fread(caminho_tabela, encoding = "UTF-8")
...
# caminho_tabela <- 
#  file.path(getwd(), "Demografia", "Atividades", "dados_sim.csv")
#  
# dados_sim %>% 
#    fwrite(file = caminho_tabela)
...
dados_sim <- fread(caminho_tabela, encoding = "UTF-8")
...
# 
caminho_tabela <- 
  file.path(getwd(), "Demografia", "Atividades", "dados_sim_2021.csv")
# 
# dados_sim_2021 %>% 
#      fwrite(file = caminho_tabela)

dados_sim_2021 <- fread(caminho_tabela, encoding = "UTF-8")
